...
mod tests {
    ...
    #[test]
    fn controller_exact_match() {
        assert!(is_authorized("kcore-controller", &[CN_CONTROLLER]));
        assert!(!is_authorized("kcore-controller-evil", &[CN_CONTROLLER]));
    }
    ...
    fn multiple_allowed_patterns() {
        let allowed = &[CN_CONTROLLER, CN_KCTL];

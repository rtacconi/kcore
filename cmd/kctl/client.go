...
func NewNodeClient(address string, tlsInsecure bool, certFile, keyFile, caFile string) (*NodeClient, error) {
	...
	return &NodeClient{
		conn:    conn,
		compute: pb.NewNodeComputeClient(conn),
		storage: pb.NewNodeStorageClient(conn),
		info:    pb.NewNodeInfoClient(conn),
	}, nil
}
...
func (c *NodeClient) GetVM(ctx context.Context, vmID string) (*pb.VmSpec, *pb.VmStatus, error) {
	resp, err := c.compute.GetVm(ctx, &pb.GetVmRequest{
		VmId: vmID,
	})
	if err != nil {
		return nil, nil, err
...
func (c *NodeClient) GetNodeInfo(ctx context.Context) (*pb.GetNodeInfoResponse, error) {
	return c.info.GetNodeInfo(ctx, &pb.GetNodeInfoRequest{})
}
